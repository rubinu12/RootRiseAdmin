...
import { topics, prelimQuestions, prelimQuestionStatements, prelimQuestionPairs, prelimQuestionTopics } from '@/lib/schema';
import { recursiveVectorSearch } from './recursive-search';
...
export async function analyzeQuestionJson(jsonData: any) {
  try {
    ...
    if (qData.statements && qData.statements[0]) searchText += " " + qData.statements[0].text;
...
export async function createTopicFromJson(subject: string, anchor: string, detailed: string) {
  try {
    // 1. Find or Create Subject (L1)
    let l1 = await db.query.topics.findFirst({ where: eq(topics.name, subject) });
    if (!l1) return { success: false, error: `Subject L1 '${subject}' not found. Please create root manually first.` };
...
export async function saveFinalQuestion(payload: {
  ...
  try {
    ...
    const [insertedQ] = await db.insert(prelimQuestions).values({
      year: meta.year,
      source: meta.source,
